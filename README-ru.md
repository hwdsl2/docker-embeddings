...
- Опциональная аутентификация по API-ключу (`EMBED_API_KEY`)
...
Также можно передать его через `--env-file`:
